...
do
 ...
 echo "fetching $URL"
  (echo "BEGIN:VCALENDAR\nVERSION:2.0\nPRODID:-//Hockeytech//Leaguestat iCal//EN\nMETHOD:PUBLISH";
   curl -s $(curl -s "$URL" | jq -R -s -r '.[1+index("("): rindex(")")] | fromjson | .[0].sections[0].data | .[] | select(.prop.mobile_calendar) | .prop.mobile_calendar.link') | awk -v FS=: -v OFS=: '/^BEGIN:VEVENT/{flag=1};$1 == "DESCRIPTION" {$2="https://www.youtube.com/@thepwhlofficial/streams"}; flag;/^END:VEVENT/{flag=0}'
   echo "END:VCALENDAR") > season-$SEASON.ical
done
